    ...
    private void handleSingletonGroups(Square square, Cycles cycles, Cycle cycle, List<Group> singletonGroups) {
        if (!singletonGroups.isEmpty()) {
            if (singletonGroups.size() > 1) {
                ...
                log.debug("{} {} add singleton group {} to random group in cycle {}",cycles.getSquare().getFillAlgorithm(), cycles.getSquare().getNumberOfElements(),  singletonGroups.get(0), cycles.size());
